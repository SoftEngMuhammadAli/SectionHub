...
export function LoginForm() {
    const [state, formAction, pending] = useActionState(loginActionState, initialState);
    return (<form action={formAction} className="space-y-5">
      ...
      <label className="block space-y-2">
        ...
        <input name="password" type="password" autoComplete="current-password" placeholder="Enter your password" className="sectionhub-input"/>
      </label>
      ...
      {state.error ? (<div className="rounded-[10px] border border-[var(--danger)]/15 bg-[var(--danger-light)] px-3 py-2 text-[12px] text-[var(--danger)]">
          {state.error}
        </div>) : null}
      <button type="submit" disabled={pending} className="inline-flex min-h-11 w-full items-center justify-center rounded-[8px] bg-[var(--primary)] px-4 text-[13px] font-medium text-white disabled:cursor-not-allowed disabled:opacity-60">
