...
async function withSectionRelations(
  query: ReturnType<typeof SectionModel.find>,
) {
...
export async function getDashboardData() {
  ...
  const totalRevenue = paidOrders.reduce(
    (sum, order: any) => sum + (order.totalAmountCents ?? 0),
    0,
  );
  ...
  return {
    overview: {
      ...
      totalInstalls: installs.filter((item: any) => item.status === "SUCCESS")
        .length,
