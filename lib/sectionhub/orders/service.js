...
export async function getOrders() {
    ...
    return orders.map((order) => ({
        ...
        customer: customers.find((item) => toId(item._id) === toId(order.customerId))?.name ?? "Unknown",
        shop: shops.find((item) => toId(item._id) === toId(order.shopId))?.domain ?? "-",
        amount: formatPrice(order.totalAmountCents),
        provider: order.paymentProvider,
        status: order.status,
        date: formatDate(order.purchasedAt),
        purchasedItems: (order.items ?? []).map((item) => item.titleSnapshot).join(", "),
    }));
}
