...
import { formatCount, formatDate, formatPrice, toId } from "@/lib/sectionhub/shared/format";
...
export async function getSections(params) {
    ...
    return sections
        .filter((section) => !params?.category || section.categoryId?.slug === params.category)
        .filter((section) => !params?.tag || (section.tagIds ?? []).some((tag) => tag.slug === params.tag))
        .map((section) => mapSectionRow(section, installMap.get(toId(section._id)) ?? 0));
}
...
export async function createOrUpdateSection(input) {
    ...
    if (input.version) {
        ...
        if (input.id)
            data.$push = { versions: versionEntry };
        else
            data.versions = [versionEntry];
    }
    return input.id
...
export async function publishSection(id) {
    ...
    return SectionModel.findByIdAndUpdate(id, {
        status: "PUBLISHED",
