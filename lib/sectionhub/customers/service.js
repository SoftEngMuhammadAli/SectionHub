...
export async function getCustomers() {
    ...
    return customers.map((customer) => {
        const customerShops = shops.filter((shop) => toId(shop.customerId) === toId(customer._id));
        const spent = orders
            .filter((order) => toId(order.customerId) === toId(customer._id))
            .reduce((sum, order) => sum + (order.totalAmountCents ?? 0), 0);
        const installCount = installs.filter((install) => customerShops.some((shop) => toId(shop._id) === toId(install.shopId))).length;
        return {
