...
import { CategoryModel, InstallEventModel, SectionModel } from "@/models";
import { formatDate, toId } from "@/features/shared/format";
...
export async function getCategories() {
  ...
  return categories.map((category: any) => {
    const scoped = sections.filter(
      (section: any) => section.categoryId && toId(section.categoryId) === toId(category._id),
    );
    const installTotal = installs.filter((install: any) =>
      scoped.some((section: any) => toId(section._id) === toId(install.sectionId)),
    ).length;
