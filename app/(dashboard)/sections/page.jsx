...
export default async function SectionsPage({ searchParams, }) {
    ...
    return (<div className="space-y-6">
      ...
      <Card className="p-4">
        <form className="grid gap-3 md:grid-cols-2 xl:grid-cols-[1.8fr_repeat(4,1fr)_auto]">
          ...
          <div className="flex gap-2">
            <button type="submit" className="inline-flex min-h-11 flex-1 items-center justify-center rounded-[8px] bg-[var(--primary)] px-4 text-[13px] font-medium text-white">
              Apply
            </button>
            <Link href="/sections" className="inline-flex min-h-11 flex-1 items-center justify-center rounded-[8px] border border-[var(--border)] bg-white px-4 text-[13px] font-medium text-[var(--text-primary)]">
              Reset
            </Link>
          </div>
        </form>
      ...
      <div className="text-[13px] text-[var(--text-secondary)]">
        {filteredSections.length} result(s)
      </div>

      <div className="sectionhub-mobile-list md:hidden">
        {filteredSections.map((section) => (<Card key={section.id} className="p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="rounded-full bg-[var(--primary-light)] px-2.5 py-1 text-[11px] font-medium text-[var(--primary-light-text)]">
                {section.status}
              </div>
            </div>
            <div className="mt-4 space-y-2 text-[13px] text-[var(--text-secondary)]">
              <div className="flex justify-between gap-3">
                <span>Category</span>
                <span className="text-right">{section.category}</span>
              </div>
              <div className="flex justify-between gap-3">
                <span>Tags</span>
                <span className="text-right">
                  {section.tags.join(", ") || "ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â"}
                </span>
              </div>
              <div className="flex justify-between gap-3">
                <span>Price</span>
                <span className="font-mono">{section.price}</span>
              </div>
              <div className="flex justify-between gap-3">
                <span>Version</span>
                <span className="font-mono">{section.version}</span>
              </div>
              <div className="flex justify-between gap-3">
                <span>Installs</span>
                <span className="font-mono">{section.installs}</span>
              </div>
              <div className="flex justify-between gap-3">
                <span>Updated</span>
            ...
            <div className="mt-4 flex gap-2">
              <Link href={`/sections/${section.id}/edit`} className="inline-flex min-h-10 flex-1 items-center justify-center rounded-[8px] border border-[var(--border)] bg-white px-3 text-[13px] font-medium text-[var(--text-primary)]">
                Edit
              </Link>
              <button type="button" className="inline-flex min-h-10 flex-1 items-center justify-center rounded-[8px] border border-[var(--danger)]/20 bg-white px-3 text-[13px] font-medium text-[var(--danger)]">
                Delete
              </button>
            </div>
          </Card>))}
        {!filteredSections.length ? (<Card className="p-6 text-[13px] text-[var(--text-secondary)]">
      ...
      <Card className="hidden overflow-hidden md:block">
        <div className="overflow-x-auto">
          <table className="min-w-[1100px] text-left text-[13px]">
            ...
            <tbody>
              {filteredSections.map((section) => (<tr key={section.id} className="border-t border-[var(--border)]">
                  ...
                  <td className="px-5 py-4">
                    <Link className="text-[var(--primary)]" href={`/sections/${section.id}/edit`}>
                      Edit
                    </Link>
                  </td>
                </tr>))}
              {!filteredSections.length ? (<tr>
                  <td colSpan={9} className="px-5 py-10 text-center text-[13px] text-[var(--text-secondary)]">
                    No sections match the current filters.
                  </td>
                </tr>) : null}
            </tbody>
