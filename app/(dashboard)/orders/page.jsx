...
export default async function OrdersPage({ searchParams, }) {
    ...
    return (<div className="space-y-6">
      ...
      <Card className="p-4">
        <form className="grid gap-3 md:grid-cols-2 xl:grid-cols-[1.8fr_1fr_1fr_auto]">
          ...
          <select name="provider" defaultValue={provider} className="sectionhub-select">
            ...
            {providers.map((item) => (<option key={item} value={item}>
                {item}
              </option>))}
          </select>
          <div className="flex gap-2">
            ...
            <a href="/orders" className="inline-flex min-h-11 flex-1 items-center justify-center rounded-[8px] border border-[var(--border)] bg-white px-4 text-[13px] font-medium text-[var(--text-primary)]">
              Reset
            </a>
          </div>
        </form>
      ...
      <div className="text-[13px] text-[var(--text-secondary)]">
        {filteredOrders.length} result(s)
      </div>

      <div className="sectionhub-mobile-list md:hidden">
        {filteredOrders.map((order) => (<Card key={order.id} className="p-4">
            ...
            <div className="mt-4 space-y-2 text-[13px] text-[var(--text-secondary)]">
              ...
              <div className="flex justify-between gap-3">
                <span>Shop</span>
                <span className="font-mono text-right">{order.shop}</span>
              </div>
              <div className="flex justify-between gap-3">
                <span>Items</span>
                <span className="text-right">{order.purchasedItems}</span>
              </div>
              <div className="flex justify-between gap-3">
                <span>Amount</span>
        ...
        {!filteredOrders.length ? (<Card className="p-6 text-[13px] text-[var(--text-secondary)]">
      ...
      <Card className="hidden overflow-hidden md:block">
        <div className="overflow-x-auto">
          <table className="min-w-full text-left text-[13px]">
            ...
            <tbody>
              {filteredOrders.map((order) => (<tr key={order.id} className="border-t border-[var(--border)]">
