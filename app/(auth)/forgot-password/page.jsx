...
export default async function ForgotPasswordPage({ searchParams }) {
    ...
    return (<AuthShell title="Forgot Password" subtitle="Recover admin access with a time-limited reset link.">
      <form action={forgotPasswordAction} className="space-y-5">
        ...
        {sent ? (<div className="rounded-[10px] bg-[var(--primary-light)] p-4 text-[12px] text-[var(--primary-light-text)]">
            Reset token generated locally. Use <span className="font-mono">{token}</span> on the reset screen.
          </div>) : null}
      </form>
    </AuthShell>);
}
