...
export async function resetPasswordAction(formData: FormData) {
  ...
  if (password.length < 8 || password !== confirmPassword) redirect("/reset-password?error=Password+validation+failed");
  const result = await resetPassword(token, password);
  if (!result.ok) redirect(`/reset-password?error=${encodeURIComponent(result.error)}`);
...
export async function saveCategoryAction(formData: FormData) {
  ...
  await createActivityLog({ actorId: String(user._id), actorName: user.name, action: category?.createdAt?.getTime?.() === category?.updatedAt?.getTime?.() ? "created" : "updated", entityType: "category", entityId: String(category?._id), entityLabel: category?.name ?? "Category" });
...
export async function saveTagAction(formData: FormData) {
  const user = await getCurrentUser();
  if (!user) redirect("/login");
  const tag = await upsertTag({ id: String(formData.get("id") || "") || undefined, name: String(formData.get("name") || ""), slug: String(formData.get("slug") || ""), color: String(formData.get("color") || "violet") });
  await createActivityLog({ actorId: String(user._id), actorName: user.name, action: "saved", entityType: "tag", entityId: String(tag?._id), entityLabel: tag?.name ?? "Tag" });
  revalidatePath("/tags");
...
export async function saveBundleAction(formData: FormData) {
  const user = await getCurrentUser();
  if (!user) redirect("/login");
  const sectionIds = String(formData.get("sectionIds") || "").split(",").map((value) => value.trim()).filter(Boolean);
  ...
  if (bundle) await createActivityLog({ actorId: String(user._id), actorName: user.name, action: "saved", entityType: "bundle", entityId: String(bundle._id), entityLabel: bundle.name });
...
export async function saveSettingsAction(formData: FormData) {
  const user = await getCurrentUser();
  if (!user) redirect("/login");
  await updateSettings({ siteName: String(formData.get("siteName") || "SectionHub Enterprise"), defaultCurrency: String(formData.get("defaultCurrency") || "USD"), maintenanceMode: asBool(formData.get("maintenanceMode")), updatedById: String(user._id) });
  await createActivityLog({ actorId: String(user._id), actorName: user.name, action: "updated", entityType: "setting", entityId: "global", entityLabel: "System Settings" });
...
export async function saveSectionAction(formData: FormData) {
  ...
  const tagIds = String(formData.get("tagIds") || "").split(",").map((value) => value.trim()).filter(Boolean);
  ...
  const section = await createOrUpdateSection({
    ...
    tagIds,
    previews: previewUrl ? [{ type: "IMAGE", url: previewUrl, title: String(formData.get("name") || "Preview"), altText: String(formData.get("name") || "Preview"), isThumbnail: true, sortOrder: 1 }] : [],
  });
  await createActivityLog({ actorId: String(user._id), actorName: user.name, action: id ? "updated" : "created", entityType: "section", entityId: String(section?._id), entityLabel: section?.name ?? "Section" });
...
export async function publishSectionAction(formData: FormData) {
  ...
  await createActivityLog({ actorId: String(user._id), actorName: user.name, action: "published", entityType: "section", entityId: String(section?._id), entityLabel: section?.name ?? "Section" });
  revalidatePath("/sections");
